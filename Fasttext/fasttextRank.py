...
def rank(queries, queryIDs, corpus_vectors, model, name='', N=1000, ):
    ...
    out.to_csv(f'{name}_RankingResults.csv', sep=',')
...
if __name__ == '__main__':
    ...
    # SBERT vectors
    try:
        SBERTCorpus = pd.read_csv(f'SBERTCorpus.csv', index_col=0).values
    except FileNotFoundError:
        SBERTCorpus = SBERT.encode(corpus)
        pd.DataFrame(ftCorpus).to_csv(f'SBERTCorpus.csv')
    print(f'Calculating all SBERT passage vectors took {time() - t:.3f} seconds')

    N = 1000
    rank(queries, queryIDs, ftCorpus, ft, name='ft', N=N)
    rank(queries, queryIDs, SBERTCorpus, SBERT, name='SBERT', N=N)
